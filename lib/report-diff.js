...
function assertOptions(options) {
    ...
    if (!_.includes(modes, options.mode)) {
        throw new Error('options.mode must be one of: ' + modes.join(', '));
    }
    
    var types = ['text'];
    
    if (!_.includes(types, options.type)) {
        throw new Error('options.type must be one of: ' + types.join(', '));
    }
}
