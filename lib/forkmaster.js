...
function start(opts, callback) {
    ...
    function writeReport(report) {
        if (absoluteStart === undefined) {
            absoluteStart = report.report.fullTest.start;
        }
        
        // adjust the start and end times based on the first start
        _.forEach(report.report, function(rep, name) {
            rep.start = rep.start - absoluteStart;
            rep.end = rep.end - absoluteStart;
        });
...
//        worker.on('exit', console.log.bind(console, 'exit'));
    ...
    function runConcurrently(context, done) {
        ...
        function startNew() {
            ...
            console.log('starting new, currently running', running);
