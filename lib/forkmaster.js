...
function start(opts, callback) {
    ...
    var handlers = {
        onTestFatalError: function onTestFatalError(err) {
            output.end();
    ...
    try {
        ...
        return handlers.onTestFatalError(e);
    }
    
    // Make sure test module has the required properties
    if (!_.isFunction(testModule.test)) {
        return handlers.onTestFatalError(new Error('no test method was found in ' + opts.filepath));
    }
...
//        worker.on('exit', console.log.bind(console, 'exit'));
    ...
    function createTestsTask() {
        ...
        var api = runner({
            ...
            debug: util.debug,
            options: opts,
            writeOutput: onReport
        });
        
        api.on('task:run', runNew);
