...
var runRateMode = require('./run-rate-mode.js');
var runConcurrentMode = require('./run-concurrent-mode.js');
...
function start(opts, callback) {
    ...
    function onThreadReady(msg, id) {
        ready += 1;
        
        var slaveName = ipc.slaveName(id);
        
        ipcApi.connect(slaveName, function() {
...
//        worker.on('exit', console.log.bind(console, 'exit'));
    ...
    var ipcApi = ipc(ipc.masterName(), function(err) {
        if (err) {
            return callback(err);
        }
...
//    var msgCount = 0;
//    ipcApi.on('message', function(msg) {
//        msgCount += 1;
...
//    });
    ...
    function runNew(context) {
        ...
        console.log('staring new', running);
...
//        });
    ...
    function runAtRate() {
        return runRateMode(runArgs());
    }
    
    function runConcurrently() {
        return runConcurrentMode(runArgs());
    }
