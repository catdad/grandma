...
function runCommand(argv) {
    return argv.command('run <testname>', 'Run a test by name.', function(yargs) {
        return addCommonCommand(yargs, 'grandma run <testname> --duration=2m --rate=200')
            .option('duration', {
                alias: 'd',
                type: 'string',
...
function reportCommand(argv) {
    return argv.command('report <glob>', 'Summarize the content of report files.', function(yargs) {
        return addCommonCommand(yargs, 'grandma report *.txt --type=text')
            .option('type', {
                alias: 't',
                type: 'string',
...
function helpCommand(argv) {
    return argv.command('help', 'Print this help message.');
}
