...
function decorateTest(testFunc, timeout, context) {
    return function(done) {
        ...
        function onDone(err) {
            ...
            setImmediate(done, undefined, timer.report(
...
function TestRunner(msg, ipcApi) {
...
//            ipcApi.send(ipc.masterName(), {
//                type: 'report',
//                report: data[reportIdx],
//                id: workerId,
//                from: ipc.slaveName(workerId)
//            });
        });
    }
...
function initForkWorker(msg) {
    var name = ipc.slaveName(workerId);
    var ipcApi;
    
    async.series([
        function setupSocket(next) {
            ipcApi = ipc(name, next);
            testInstance = TestRunner(msg, ipcApi);
        },
        function connectToMaster(next) {
            ipcApi.connect(ipc.masterName(), next);
        }
    ], function() {
        reporter.ready();
    });
