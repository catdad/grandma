...
module.exports = function(opts, done) {
    ...
    function onJson(data) {
        if (data.type === 'report') {
            ...
            output.write(serializeReport(data));
        }
    }
    ...
    async.auto({
        build: function(next) {
            ...
            vfs.src(REPORT_FILE)
                .pipe(include())
                .on('error', next)
                .pipe(readFs(function onFile(content, file, stream, cb) {
        ...
        view: ['build', function(result, next) {
            var html = result.build;
