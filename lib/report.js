...
function mergeHeaderJson(header, data) {
    ...
    if (header.rate !== undefined && !_.isArray(header.rate)) {
        header.rate = [header.rate];
    }

    if (header.rate) {
        header.rate.push(data.rate);
    } else {
        header.rate = [data.rate];
    }
    
    if (header.concurrent !== undefined && !_.isArray(header.concurrent)) {
        header.concurrent = [header.concurrent];
    }
    
    if (header.concurrent) {
        header.concurrent.push(data.concurrent);
    } else {
        header.concurrent = [data.concurrent];
    }
...
function Reporter() {
    ...
    function readJsonStats(input, done) {
        ...
        readJsonLines(input, onJson, function(err) {
            ...
            header.rate = stats.mean(header.rate);
            header.concurrent = stats.mean(header.concurrent);
