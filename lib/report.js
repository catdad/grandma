...
function Reporter(repOpts) {
    ...
    function jsonStats(data) {
        ...
        statsJson.categories = _.reduce(categories, function(memo, arr, name) {
            var group = {
                info: {
                    count: arr.length
                }
    ...
    function readJsonStats(input, done) {
        ...
        function onJson(data) {
            ...
            if (data.type === 'header') {
                ...
                var reportSummary = _.reduce(data.report, function(seed, rep, name) {
                ...
                if (data.categories && data.categories.length) {
                    data.categories.forEach(function(name) {
                        insertInArray(categories, name, reportSummary);
                    });
                }
    ...
    function textStats(jsonSummary) {
        ...
        tableArr.push(serializer.textArr.latenciesTitle('Latencies'));
        
        // add latencies for each report
        _.forEach(jsonSummary.latencies, function(latencies, name) {
            tableArr.push(serializer.textArr.latencies(latencies, name));
        });
        ...
        if (jsonSummary.categories && Object.keys(jsonSummary.categories).length) {
            ...
            _.forEach(jsonSummary.categories, function(category, categoryName) {
                tableArr.push([]);
                tableArr.push(['Category: ' + categoryName]);
    ...
    this.plot = function plotReporter(input, output, done) {
        ...
        async.auto({
            ...
            readJson: ['inputs', function(results, next) {
                readJsonStats(results.inputs.jsonStream, function(err, statsObj) {
                    if (err) {
                        return next(err);
                    }
                    
                    var text = textStats(statsObj);
